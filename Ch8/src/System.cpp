    ...
    :bStart_backend(true)
{
    ...
    system("rm ./pose_output.txt");// 清空上一次的pose文件
...
void System::PubSimFeature(double dStampSec, const vector<int> &pt_ids, const vector<Vector2d> &cur_un_pts,
                const vector<Vector2d> &cur_pts, const vector<Vector2d> &cur_velocity)
{
    shared_ptr<IMG_MSG> feature_points(new IMG_MSG());
    feature_points->header = dStampSec;
    vector<set<int>> hash_ids(NUM_OF_CAM);
    for (int i = 0; i < NUM_OF_CAM; i++)
    {
        ...
        for (unsigned int j = 0; j < pt_ids.size(); j++)
        {
            ...
            int p_id = pt_ids[j];
            hash_ids[i].insert(p_id);
            double x_value = cur_un_pts[j].x();
            double y_value = cur_un_pts[j].y();
            double z = 1;
            ...
            feature_points->u_of_point.push_back(cur_pts[j].x());
            feature_points->v_of_point.push_back(cur_pts[j].y());
            feature_points->velocity_x_of_point.push_back(cur_velocity[j].x());
            feature_points->velocity_y_of_point.push_back(cur_velocity[j].y());
        }
        // skip the first image; since no optical speed on frist image
        if (!init_pub)
        {
...
{   
    ...
    while (pangolin::ShouldQuit() == false) {
        ...
        glColor3f(0, 0, 1);
        pangolin::glDrawAxis(3);// 绘制坐标轴，参数为坐标轴长度，原点默认在(0,0,0)
         
        // draw poses (estimator)
        glColor3f(0, 0, 0);
        glLineWidth(2);
        glBegin(GL_LINES);
        ...
        glEnd();
        // draw poses (ground_truth)
        glColor3f(0., 1., 1.);
        glBegin(GL_LINES);
        for(int i=0; i<pos_gt_.size() - 1;i++){
            glVertex3f(pos_gt_[i].x(), pos_gt_[i].y(), pos_gt_[i].z());
            glVertex3f(pos_gt_[i+1].x(), pos_gt_[i+1].y(), pos_gt_[i+1].z());
        }
        glEnd();
        // points
        if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
