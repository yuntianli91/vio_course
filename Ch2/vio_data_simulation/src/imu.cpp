...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    /// 中值积分初始值
    Eigen::Vector3d acc_0 = imudata[0].imu_acc;
    Eigen::Vector3d gyr_0 = imudata[0].imu_gyro;
    /// 积分解算
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        /// =========================================== 中值积分 ============================================== ///
        Eigen::Vector3d acc_1 = imudata[i].imu_acc;
        Eigen::Vector3d gyr_1 = imudata[i].imu_gyro;
        // 姿态四元数更新
        Eigen::Vector3d dtheta_half = 0.25 * (gyr_0 + gyr_1) * dt;
        Eigen::Quaterniond dq;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        ...
        Eigen::Quaterniond Qwb0 = Qwb;
        Qwb = Qwb0 * dq;
        // 位姿更新
        Eigen::Vector3d acc_w = 0.5 * (Qwb * acc_1 + Qwb0 * acc_0) + gw;
