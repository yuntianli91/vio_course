...
/*
 * 产生世界坐标系下的虚拟数据: 相机姿态, 特征点, 以及每帧观测(相机仰视，所有特征点在相机上面)
 */
...
int main(int argc, char** argv) {
    if(argc != 2){
        cout << "Wrong input argument list !" << endl
            << "usage: testMonoBA w_p" << endl;
        return -1;
    }
    // 存储函数
    ofstream out_poses("poses.txt", ios::app);
    ofstream out_quat("quat.txt", ios::app);
    // 循环50次统计误差
    for(int i = 0; i< 50; i++){
        // 准备数据
    ...
    //        double inverse_depth = 1. / Pc.z();
        ...
        // 第一、二帧先验边
        for(int i=0; i<2; i++){
            shared_ptr<EdgeSE3Prior> edge(new EdgeSE3Prior(cameras[i].twc, cameras[i].qwc, atof(argv[1])));
            std::vector<std::shared_ptr<Vertex>> edge_vertex;
        ...
        // 设置先验权重
        // problem.setPriorWeight(atof(argv[1]));
        // 求解
        problem.Solve(10);
        ...
        for (int i = 0; i < vertexCams_vec.size(); ++i) {
            out_poses << vertexCams_vec[i]->Parameters()(0) << " " 
                    << vertexCams_vec[i]->Parameters()(1) << " "
                    << vertexCams_vec[i]->Parameters()(2) << " ";

            out_quat << vertexCams_vec[i]->Parameters()(6) << " "   
                    << vertexCams_vec[i]->Parameters()(3) << " "
                    << vertexCams_vec[i]->Parameters()(4) << " "
                    << vertexCams_vec[i]->Parameters()(5) << " ";
            // cout << "gt: " << cameras[i].qwc.w() << " " << cameras[i].qwc.vec().transpose() << "\n";
        }
        out_poses << "\n";
        out_quat << "\n";
