...
bool Problem::Solve(int iterations) {
    ofstream out_solver("solver.txt", ios::app);
...
//                stop = (b_max <= 1e-12);
    ...
    double t_solve_cost = t_solve.toc();
    out_solver << (iter + total_false_cnt) << " " << t_solve_cost << "\n";
    out_solver.close();
    std::cout << "problem solve cost: " << t_solve_cost << " ms" << std::endl;
...
void Problem::MakeHessian() {
    TicToc t_h;
    // 直接构造大的 H 矩阵
    ulong size = ordering_generic_; // 获取待优化参数的总维数
    MatXX H(MatXX::Zero(size, size)); //空的Hessian矩阵H
    VecX b(VecX::Zero(size)); // 空的残差矢量b

    for (auto &edge: edges_) {
        // Edges为哈希数组[id, Edge]
        edge.second->ComputeResidual(); //计算残差r
        edge.second->ComputeJacobians(); // 计算Jacobian矩阵J
        ...
        for (size_t i = 0; i < verticies.size(); ++i) {
            ...
            if (v_i->IsFixed()) continue;    // 固定不需要优化的顶点， Hessian 里不需要添加它的信息，也就是它的雅克比为 0

            auto jacobian_i = jacobians[i];
            ulong index_i = v_i->OrderingId(); //i顶点序号
            ulong dim_i = v_i->LocalDimension(); //i顶点维数

            MatXX JtW = jacobian_i.transpose() * edge.second->Information(); //顶点对应的协方差矩阵
            for (size_t j = i; j < verticies.size(); ++j) {
                ...
                ulong index_j = v_j->OrderingId(); //j顶点序号
                ulong dim_j = v_j->LocalDimension();//j顶点维数

                assert(v_j->OrderingId() != -1);
                MatXX hessian = JtW * jacobian_j;
                // 所有的信息矩阵叠加起来(noalias()函数是Eigen用以解决矩阵运算混淆问题的)
                // TODO:: home work. 完成 H index 的填写.
                // 这里完成了H矩阵上三角部分的遍历赋值
                H.block(index_i, index_j, dim_i, dim_j).noalias() += hessian;
                // 对于非对角线元素，还需完成对称的下三角部分赋值 
                if (j != i) {
		        // TODO:: home work. 完成 H index 的填写.
    ...
    // 固定前两维位姿
    // double w_p = 1e5; // 先验权重
    // Hessian_.block(0, 0, 12, 12) += w_p_ * Eigen::MatrixXd::Identity(12, 12);
    // cout << "Add prior weight " << w_p_ << " to Hessian.\n";
    b_ = b;
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        MatXX Hmm = Hessian_.block(reserve_size, reserve_size, marg_size, marg_size); // 右下部分信息矩阵
        MatXX Hpm = Hessian_.block(0, reserve_size, reserve_size, marg_size); //右上部分信息矩阵
        MatXX Hmp = Hessian_.block(reserve_size, 0, marg_size, reserve_size); //左下部分信息矩阵
...
void Problem::TestMarginalize() {
    ...
    // 为何不直接是 Eigen::MatrixXd Amm = H_marg.block(n2, n2, m2, m2)？ ;
